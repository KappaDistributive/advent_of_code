...
long part_two(const std::vector<std::string>& input)
{
  ...
  long max = jolts[jolts.size() - 1];
  paths.reserve(max +  1);
  paths[0] = 1;
  paths[1] = static_cast<long>(indicator(1, jolts));
  paths[2] = static_cast<long>(indicator(2, jolts) * (paths[0] + paths[1]));

  for (size_t index{3}; index < max + 1; index++)
  {
    paths[index] = static_cast<long>(indicator(index, jolts) * (paths[index-3] + paths[index-2] + paths[index-1]));
  }
  return paths[max];
}
