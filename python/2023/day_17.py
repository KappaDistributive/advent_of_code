...
def part_two(data: list[list[int]]) -> int:
    ...
    while pq:
        ...
        # Check if we reached the destination (must have moved at least 4 blocks)
        if r == rows - 1 and c == cols - 1 and consecutive >= 4:
            return heat_loss
        ...
        # Try moving in the same direction (if we haven't moved 10 times already)
        if consecutive < 10:
            dr, dc = directions[direction]
            nr, nc = r + dr, c + dc
            if 0 <= nr < rows and 0 <= nc < cols:
        ...
        # Try turning left or right (only if we've moved at least 4 blocks)
        if consecutive >= 4:
            for turn in [-1, 1]:
                new_direction = (direction + turn) % 4
                dr, dc = directions[new_direction]
