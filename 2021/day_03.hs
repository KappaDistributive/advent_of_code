...
mostCommonDigit n x d = if uncurry (==) counts then d else uncurry (<) counts
  where counts = digitsCounts n x

leastCommonDigit n x d = if uncurry (==) counts then d else uncurry (>) counts
  where counts = digitsCounts n x
...
oxygenGeneratorRating pos x = if pos == length (head x) then [] else digit : oxygenGeneratorRating next_pos output
  where
...
scrubberRating pos x
  ...
  where (next_pos, output, digit) = scrubberRating' pos x
