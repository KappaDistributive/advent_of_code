...
public:
  Path (const std::string& passcode)
    : open_door_codes("bcdef"), passcode(passcode), path(), position({0, 0})
  {
  }

  Path (const Path& other)
    : open_door_codes(other.open_door_codes), passcode(other.passcode), path(other.path), position(other.position)
  {
  }
  ...
  std::string path_to_string(const std::vector<Direction>& path, size_t offset=0) const
  {
    if (offset >= path.size())
    {
      return "";
    ...
    switch (path[offset])
    {
      case up: path_str += 'U'; break;
      case down: path_str += 'D'; break;
      case left: path_str += 'L'; break;
      case right: path_str += 'R'; break;
    }
  ...
  {
    return path_to_string(this->path);
  }

  bool valid_move (const Direction& direction)
  {
    if (
        (this->position.first == 0 && direction == left) ||
    ...
    switch (direction)
    {
      case up:    open_door = open_door_codes.find(md5(this->passcode + path_to_string())[0]) != std::string::npos; break;
      case down:  open_door = open_door_codes.find(md5(this->passcode + path_to_string())[1]) != std::string::npos; break;
      case left:  open_door = open_door_codes.find(md5(this->passcode + path_to_string())[2]) != std::string::npos; break;
      case right: open_door = open_door_codes.find(md5(this->passcode + path_to_string())[3]) != std::string::npos; break;
    }
  ...
  bool step(const Direction& direction)
  {
    ...
    switch (direction)
    {
      case up: this->position.second--; break;
      case down: this->position.second++; break;
      case right: this->position.first++; break;
      case left: this->position.first--; break;
    }
