...
partOne (Just rooms) =
  ...
  map (\(_, _, a) -> a) $
  filter (\(a, b, _) -> a == b) $
  map (\room@(Room _ sector_id c) -> (c, checksum room, sector_id)) rooms
...
partTwo (Just rooms) =
  Just $
  snd . head $
  filter (\(a, b) -> a == "northpole-object-storage") $
  map (\(Room name sectorId _) -> (decrypt sectorId name, sectorId)) $ rooms
