...
triplets True (a:b:c:d:e:f:g:h:i:xs) =
  (read a, read d, read g) :
  (read b, read e, read h) : (read c, read f, read i) : triplets True xs
triplets _ _ = []
...
countTriangles candidates =
  length $ filter (== True) $ map isTriangle candidates
