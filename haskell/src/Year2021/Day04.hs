...
parseMoves =
  map fst .
  Data.Either.rights .
  map decimal . T.splitOn (T.pack ",") . T.pack . head . lines
...
parseBoard x = Board numbers state (-1)
  where
    numbers =
      map fst .
      Data.Either.rights . map decimal . T.split (\c -> c == ' ' || c == '\n') $
      x
...
score (Board numbers state winning_number) =
  winning_number * sum (map fst (filter (not . snd) (zip numbers state)))

winningMove :: [Int] -> Board -> Int
winningMove _ (Board _ _ (-1)) = -1
winningMove moves (Board _ _ winning_number) =
  case index of
    Just i -> i
    Nothing -> -1
  where
...
hasWon :: Board -> Bool
hasWon board =
  any ((== True) . (`completeRow` board)) [0, 1, 2, 3, 4] ||
  any ((== True) . (`completeColumn` board)) [0, 1, 2, 3, 4]

applyMove :: Int -> [Board] -> [Board]
applyMove number (x:xs) =
  Board numbers new_state new_winning_number : applyMove number xs
  where
    Board numbers state winning_number = x
    index = L.elemIndex number numbers
    new_state =
      if winning_number == -1
        then updateState index state
        else state
    new_winning_number =
      if winning_number == -1 && hasWon (Board numbers new_state winning_number)
        then number
        else winning_number
applyMove number [] = []
...
step moves boards = new_boards : step (tail moves) new_boards
  where
    new_boards = applyMove (head moves) boards
...
partTwo x = fst $ last sorted_winners
  where
    ...
    sorted_winners =
      ...
      zip (map score winners) (map (winningMove moves) winners)
