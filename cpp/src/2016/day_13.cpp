...
bool is_wall(const size_t& offset, const size_t& x, const size_t& y) {
  uint64_t value{x * x + 3 * x + 2 * x * y + y + y * y + offset};
  int ones{0};
...
void show_maze(const size_t offset, const size_t& max_x, const size_t& max_y,
               std::vector<std::pair<size_t, size_t>> path) {
  for (size_t y{0}; y < max_y; y++) {
    for (size_t x{0}; x < max_x; x++) {
      if (std::find(path.begin(), path.end(), std::make_pair(x, y)) !=
          path.end()) {
        std::cout << 'O';
      } else {
...
std::set<std::pair<size_t, size_t>> path_extensions(
    const size_t& offset, const std::vector<std::pair<size_t, size_t>>& path) {
  ...
  for (int offset_y{-1}; offset_y <= 1; offset_y++) {
    for (int offset_x{-1}; offset_x <= 1; offset_x++) {
      if ((offset_x == 0 && offset_y == 0) ||
          ...
          (offset_x < 0 && static_cast<int>(x) < -offset_x) ||
          (offset_y < 0 && static_cast<int>(y) < -offset_y)) {
        continue;
      }
      candidate_x = x + offset_x;
      candidate_y = y + offset_y;
      if (!is_wall(offset, candidate_x, candidate_y) &&
          std::find(path.begin(), path.end(),
                    std::make_pair(candidate_x, candidate_y)) == path.end()) {
...
int main() {
  utils::Reader reader(std::filesystem::path("../../data/2016/input_13.txt"));
