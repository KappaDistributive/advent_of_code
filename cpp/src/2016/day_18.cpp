...
std::tuple<char, char, char> get_predeseccors(const std::string& previous_row,
                                              size_t index) {
  ...
  right =
      index + 1 == previous_row.size() ? SAFE_TILE : previous_row[index + 1];
...
char calculate_tile(const std::string& previous_row, size_t index) {
  ...
  if ((left == TRAP_TILE && center == TRAP_TILE && right == SAFE_TILE) ||
...
uint64_t part_two(const std::string& input) {
  std::vector<std::string> rows{{input}};
  // It would suffice to calculate a single period and extrapolate from there.
  // But this is still plenty fast.
...
int main() {
  utils::Reader reader(std::filesystem::path("../../data/2016/input_18.txt"));
