...
std::vector<std::array<int, 3>> prepare_input(
    const std::vector<std::string>& input, bool part_one = true) {
  ...
  if (part_one) {
    for (auto line : input) {
      ...
      result.push_back({std::stoi(matches[1].str()),
    ...
    for (size_t index{0}; index + 2 < input.size(); index += 3) {
      ...
      result.push_back(
          {buffer[0 * 3 + 0], buffer[1 * 3 + 0], buffer[2 * 3 + 0]});
      result.push_back(
          {buffer[0 * 3 + 1], buffer[1 * 3 + 1], buffer[2 * 3 + 1]});
      result.push_back(
          {buffer[0 * 3 + 2], buffer[1 * 3 + 2], buffer[2 * 3 + 2]});
    }
  }
  return result;
}
...
int main() {
  utils::Reader reader(std::filesystem::path("../../data/2016/input_03.txt"));
