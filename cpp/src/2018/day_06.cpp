...
std::vector<point>
find_closest(const point& x, const std::vector<point>& neighbors) {
...
get_borders(const std::vector<point>& coordinates) {
...
class Map {
 ...
 public:
    ...
    explicit Map(const std::vector<point>& coordinates)
    ...
    std::optional<point>& operator[](const point& coordinate) {
        ...
        return std::ref(
            owners[coordinate.first * (max_y+1) + coordinate.second]);
    }
...
size_t part_one(const std::vector<std::string>& input) {
    ...
    for (auto coordinate : coordinates) {
        ...
        if (is_finite && land.size() > result) {
            std::cout << "New Champion: " << coordinate
                      << " with score: " << land.size()
                      << std::endl;
            result = land.size();
        }
    }
...
int main() {
  utils::Reader reader(std::filesystem::path("../../data/2018/input_06.txt"));
