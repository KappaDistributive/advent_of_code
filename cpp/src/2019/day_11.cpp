 ...
 *   - First: 0 means to paint the panel black, and 1 means to paint the panel
 * white.
 *   - Second: Second, it will output a value indicating the direction the robot
 * should turn: 0 means it should turn left 90 degrees, and 1 means it should
...
size_t part_one(const std::vector<std::string>& input) {
  ...
  if (verbose) std::cout << robot << std::endl;
  while (!halting) {
    halting = robot.step();
    if (verbose) std::cout << robot << std::endl;
  }
...
int main() {
  utils::Reader reader(std::filesystem::path("../../data/2019/input_11.txt"));
