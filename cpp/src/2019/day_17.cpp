...
class CPU {
 ...
 public:
  ...
  std::optional<int64_t> get_output() const { return output; }

  void clear_output() { output = std::nullopt; }
};

enum class Orientation : int { north, east, south, west, tumbling };
...
std::tuple<std::map<Point, char>, size_t, size_t> create_map(
    const std::vector<int64_t>& intcodes) {
...
bool is_intersection(const Point& point, const std::map<Point, char> map,
                     size_t width, size_t height) {
  bool is_intersection{true};
  Point position;
  if (map.count(point) == 0 || map.at(point) != '#' || point.first == 0 ||
      point.first + 1 >= width || point.second == 0 ||
...
std::set<Point> calculate_intersections(const std::map<Point, char> map,
                                        size_t width, size_t height) {
...
void print(const std::map<Point, char>& map, size_t width, size_t height) {
  Point position{0, 0};
  for (size_t y{0}; y < height; ++y) {
    for (size_t x{0}; x < width; ++x) {
...
auto part_one(const std::vector<std::string>& input) {
  ...
  for (auto intersection : intersections) {
    // std::cout << "Intersection at (" << intersection.first << ", " <<
    // intersection.second << ")\n";
...
int main() {
  utils::Reader reader(std::filesystem::path("../../data/2019/input_17.txt"));
