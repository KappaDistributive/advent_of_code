...
public:
  std::smatch matches;
  Map(const std::vector<std::string>& input) : re("^(\\w+)\\sto\\s(\\w+)\\s=\\s(\\d+)$")
  {
    ...
    for (auto line: input)
    {
      ...
      if (std::find(cities.begin(), cities.end(), destination) == cities.end())
      {
        cities.push_back(destination);
      }
      distances.insert(std::make_pair(std::make_pair(start, destination), distance));
    }
  ...
  int get_distance(std::string start, std::string destination) const
  {
    try
    {
      ...
      int j = std::find(cities.begin(), cities.end(), destination) - cities.begin();
  ...
  std::vector<std::string> hamiltonian_path(bool shortest = true)
  {
    ...
    do {
      ...
      {
        next_distance = get_distance(cities[indices[index-1]], cities[indices[index]]);
        if (next_distance < 0)
        {
          valid = false;
          break;
      ...
      if (valid && (distance < 0 || (shortest && candidate < distance) || (!shortest && candidate > distance)))
      {
        distance = candidate;
        permutation = indices;
      }
...
std::ostream& operator<< (std::ostream& os, const Map& map)
{
  ...
  for (auto start: cities)
  {
    for (auto destination: cities)
    {
      ...
      if (distance >= 0)
      {
        std::cout << start << " -- " << distance << " --> " << destination << std::endl; 
      }
    }
  }
  return os;
