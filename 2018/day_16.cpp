...
template<typename T>
std::ostream&
operator<<(std::ostream& os, const std::vector<T>& values) {
  os << "[";
...
get_modes(Opcode opcode) {
  switch (opcode) {
    ...
    case Opcode::seti: return {Mode::val, Mode::val}; break;
...
std::pair<uint, uint>
get_values(const Opcode& opcode, const std::vector<uint>& instruction, std::vector<uint>* registers) {
  auto modes = get_modes(opcode);
  std::pair<uint, uint> values;
  values.first = modes.first == Mode::reg ? registers->operator[](instruction[1]) : instruction[1];
  values.second = modes.second == Mode::reg ? registers->operator[](instruction[2]) : instruction[2];
...
void
run_opcode(const Opcode opcode,
...
std::vector<uint>
extract_registers(std::string input) {
